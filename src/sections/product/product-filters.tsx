    ...
    <Stack>
      ...
      {categories.map((option : any) => (
        <FormControlLabel
          ...
          control={
            <Radio
              checked={option.name === filters.category}
              onClick={() => handleFilterCategory(option.name)}
            />
          }
          label={option.name}
          sx={{
...
function InputRange({ type, value, onFilters }: InputRangeProps) {
  ...
  const handleBlurInputRange = useCallback(() => {
    ...
    if (min > MAX) {
      onFilters('priceRange', [MAX, max]);
    }
    if (max < 0) {
      onFilters('priceRange', [min, 0]);
    }
    if (max > MAX) {
      onFilters('priceRange', [min, MAX]);
    }
  }, [max, min, onFilters]);

...
// types
import { ICategory, IProduct, IProductTableFilters, IProductTableFilterValue } from 'src/types/product';
// hooks
...
const TABLE_HEAD = [
  ...
  { id: 'quantity', label: 'Tồn Kho', width: 160 },
  { id: 'price', label: 'Giá', width: 140 },
  { id: 'sold', label: 'Đã Bán', width: 110 },
  { id: '', width: 88 },
];
...
function applyFilter({
  ...
  const { name, stock, publish, category } = filters;
  ...
  if (category.length) {
    inputData = inputData.filter((product) => category.includes((product.category as ICategory).name));
  }
